...
<div style="max-width: 900px; margin: 30px auto; font-family: Arial, sans-serif;">
    ...
    <h1 style="margin-bottom: 24px; color: #1d4ed8; font-weight: 700; font-size: 2rem;">Hero Images</h1>

    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
        <div style="color: #6b7280; font-size: 14px;">
            <a href="{{ route('dashboard') }}" style="text-decoration: none; color: #3b82f6;">Dashboard</a> / 
            <span style="color: #111827;">Hero Images</span>
        </div>
    ...
    <div style="overflow-x: auto; box-shadow: 0 0 8px rgba(0,0,0,0.05); border-radius: 8px;">
        ...
        <table style="width: 100%; border-collapse: collapse; min-width: 700px;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    <td style="padding: 12px 16px;">{{ $heroImage->title ?? '-' }}</td>
                    <td style="padding: 12px 16px;">
                        @if ($heroImage->image_path)
                            <img src="{{ Storage::url($heroImage->image_path) }}" alt="{{ $heroImage->title }}" style="width: 60px; height: 60px; border-radius: 6px; object-fit: cover;">
                        @else
                            <span style="color: #6b7280; font-style: italic;">No image</span>
                        @endif
                    </td>
                    <td style="padding: 12px 16px;">{{ \Illuminate\Support\Str::limit($heroImage->description, 50) ?? '-' }}</td>
                    <td style="padding: 12px 16px; white-space: nowrap;">
        
                        <a href="{{ route('admin.hero_images.show', $heroImage) }}" title="View" 
                        ...
                        <a href="{{ route('admin.hero_images.edit', $heroImage) }}" title="Edit" 
                        ...
                        <form action="{{ route('admin.hero_images.destroy', $heroImage) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this hero image?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
        ...
        @else
            <p style="color:#6b7280; font-style: italic; padding: 16px;">No hero images found.</p>
        @endif
    </div>
</div>
@endsection